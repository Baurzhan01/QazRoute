...
function GuideRepairsPage() {
  ...
  const aggregate = (type: string) => items.filter((r) => r.repairType === type).length

  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <div>
          <h1 className="text-2xl font-bold text-sky-800">Ремонты</h1>
          <p className="text-sm text-gray-600">Обзор по выбранной дате</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent className="text-3xl font-bold">{aggregate("Planned")}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-gray-600">Неплановые</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-bold">{aggregate("Unscheduled")}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-gray-600">Длительные</CardTitle>
          </CardHeader>
          <CardContent className="text-3xl font-bold">{aggregate("LongTerm")}</CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">Нет ремонтов за выбранную дату</p>
          ) : (
...
export default function GuideRepairsPageWrapper() {
  return (
